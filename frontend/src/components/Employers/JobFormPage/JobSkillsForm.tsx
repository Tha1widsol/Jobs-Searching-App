...
import { addSkill } from '../../../features/Employers/jobs/job'
import Errors from '../../Global/messages/Errors'
import { token } from '../../../features/Auth/user'
import axios from 'axios'

export default function JobSkillsForm({isIsolated = true, edit = false, popupOff, toggleTab}: {isIsolated?: boolean, edit?: boolean, popupOff: () => void, toggleTab: () => void}) {
    ...
    useEffect(() => {
        if (!edit) return
        setSkills(prev => {return{...prev, value: job.skills}})
    ...
    function handleAddSkill(e: React.SyntheticEvent){
      ...
      if (job.skills?.filter(item => item.name === currentSkill).length > 0){
      ...
      axios.post(`/api/job/skills?id=${job.id}`,form, requestOptions)
      .then(response => {
